...
SRC_NABO = {"label": "2024년 개정세법 심의 결과 및 주요 내용 (확인 2026. 9. 24.)",
            ...
            "publisher": "국회예산정책처"}
SRC_DEDUCT = {"label": "찾기쉬운 생활법령정보 — 상속세 계산 및 납부 (확인 2026. 9. 24.)",
              "url": ("https://easylaw.go.kr/CSP/CnpClsMain.laf?popMenu=ov&csmSeq=255"
                      "&ccfNo=7&cciNo=2&cnpClsNo=1"), "publisher": "법제처"}
...
# 세액 자체는 쓰되 조언으로 읽히는 표현은 계속 막는다.
# "내야 할 세금은"처럼 확정적으로 들리는 어법도 남겨 둔다 — 우리 값은 가정 위의 추정이다.
BAN_TAXADVICE = ["절세하려면", "절세 효과", "미리 증여", "지금 증여", "증여하세요", "상속하세요",
                 "세금을 줄이려면", "유리합니다", "신고하지 않아도", "안 내도 됩니다",
                 "내야 할 세금은", "내실 세금은", "세금이 나옵니다"]
# 세액이 들어간 글에 반드시 있어야 하는 것들
NEED_WITH_TAX = ["가정", "여기서부터는 단서입니다", "세무"]
...
# 상속세 누진세율. 국세청 누진공제표와 법제처 산식이 같은 값을 내는지 교차 검증했다.
BRACKETS = [(1e8, 0.10, 0), (5e8, 0.20, 1e7), (10e8, 0.30, 6e7),
            (30e8, 0.40, 1.6e8), (float("inf"), 0.50, 4.6e8)]
LUMPSUM = 5e8   # 일괄공제. 상속세 및 증여세법 제21조 (법제처 확인 2026-09-24)


def tax_won(base_won):
    """과세표준(원) → 산출세액(원). 세액공제는 반영하지 않는다."""
    if base_won <= 0:
        return 0.0
    for cap, rate, ded in BRACKETS:
        if base_won <= cap:
            return base_won * rate - ded
    raise AssertionError("구간을 벗어났다")


def _check_brackets():
    """법제처가 적어 둔 산식과 국세청 누진공제표가 어긋나면 여기서 멈춘다."""
    def easylaw(b):
        if b <= 1e8:
            return b * 0.10
        if b <= 5e8:
            return 1e7 + (b - 1e8) * 0.20
        if b <= 10e8:
            return 9e7 + (b - 5e8) * 0.30
        if b <= 30e8:
            return 2.4e8 + (b - 10e8) * 0.40
        return 10.4e8 + (b - 30e8) * 0.50
    for t in (0.5e8, 1e8, 3e8, 5e8, 7e8, 10e8, 20e8, 30e8, 50e8, 90e8, 125e8):
        assert abs(tax_won(t) - easylaw(t)) < 1, t
...
def won_full(won):
    """원 → '22억 6,500만원'. 세액은 자릿수를 흐리지 않고 적는다."""
    man = round(won / 10000)
    ok, rest = divmod(man, 10000)
    if ok and rest:
...
def tax_text(manwon):
    """거래가(만원) → 일괄공제만 적용했을 때의 산출세액 문자열."""
    return won_full(tax_won(manwon * 10000 - LUMPSUM))
...
def facts(slug):
    ...
    return {
        ...
        # 거래가 기준과 과세표준 기준은 세는 결과가 다르다. 그 차이가 이 팩의 핵심이다.
        "price_over30": sum(1 for r in live if r["amount_manwon"] > 300000),
        "base_over30": sum(1 for r in live if r["amount_manwon"] * 10000 - LUMPSUM > 30e8),
    }
...
def posts_onebailey():
    ...
    o.append(dict(slug="one-bailey", persona="wb-persona-tax-oneline",
        title=f"가정 넷을 두고 계산하면 {tax_text(f['med'])}",
        body=f"""래미안 원베일리의 2026년 매매 신고 {f['n']}건은 {eok(f['lo'])}부터 {eok(f['hi'])}까지입니다. 가장 낮은 신고도 30억을 넘습니다.

여기에 상속세가 얼마나 붙는지 계산해 보겠습니다. 다만 가정을 먼저 적습니다. 가정이 바뀌면 답이 통째로 바뀌기 때문입니다.

하나, 물려주는 재산이 이 아파트 한 채뿐이고 채무나 장례비가 없다고 봅니다. 둘, 배우자가 없고 자녀가 물려받는다고 봅니다. 셋, 10년 안에 사전증여한 재산이 없다고 봅니다. 넷, 상속공제는 일괄공제 5억원만 적용합니다. 법제처 생활법령정보에 상속세 및 증여세법 제21조를 근거로 5억원이라고 적혀 있습니다.

이 가정에서 평가액을 중앙값 {eok(f['med'])}으로 두면 과세표준은 {eok(f['med'] - 50000)}이고, 국세청 세율표를 적용한 산출세액은 {tax_text(f['med'])}입니다. {eok(f['lo'])}이면 {tax_text(f['lo'])}, {eok(f['hi'])}이면 {tax_text(f['hi'])}이 됩니다.

여기서부터는 단서입니다. 이 숫자는 위 네 가정 위에서만 성립합니다. 배우자가 있으면 배우자상속공제가 더해져 과세표준이 내려가고 세액도 내려갑니다. 그 금액은 실제 상속분과 법정상속분 한도로 정해져서 저희가 계산하지 않았습니다. 신고세액공제 같은 세액공제도 반영하지 않았고, 평가액은 그 거래가를 그대로 쓴 값입니다. 실제로는 유사매매사례가액 규정에 따라 다른 거래가 기준이 될 수 있습니다.

그러니 이 계산은 자릿수를 가늠하는 용도입니다. 실제 신고액은 세무 대리인과 확인하셔야 합니다.

네 가정 중 본인 사정과 다른 항목이 있으신가요?""",
        note=("세율표는 국세청, 일괄공제 5억원은 법제처 생활법령정보(상증세법 제21조) · 둘 다 확인 2026. 9. 24. · "
              f"거래는 국토교통부 실거래 2026년 계약분 {f['n']}건, 2026. 8. 23. 조회 · "
              "가정: 아파트 한 채만 상속·배우자 없음·10년 내 사전증여 없음·일괄공제만 적용 · "
              "배우자상속공제와 세액공제는 반영하지 않았습니다"),
        srcs=[SRC_RATE, SRC_DEDUCT, SRC_TRADE],
        comments=[("wb-persona-appraisal-check", "가정을 계산보다 먼저 적은 순서가 맞습니다. 뒤에 붙이면 아무도 안 읽어요.", "기준 통일"),
                  ("wb-persona-question-post", "공제 금액은 각자 조건으로 국세청 홈택스와 상담센터에서 확인하실 수 있습니다. 저희 값은 가정 위의 추정입니다.", "확인 요청")]))
...
def posts_helio():
    ...
    o.append(dict(slug="helio-city", persona="wb-persona-appraisal-check",
        title=f"거래가로는 {f['price_over30']}건, 과세표준으로는 {f['base_over30']}건",
        body=f"""헬리오시티의 2026년 매매 신고 {f['n']}건 가운데 거래가가 30억을 넘은 것은 {f['price_over30']}건입니다. 30억을 세는 이유는 상속세 세율표에서 과세표준 30억 초과 구간이 50%로 올라가기 때문입니다.

그런데 그 경계는 거래가가 아니라 과세표준에 걸립니다. 실제로 빼 보면 숫자가 달라집니다.

가정은 넷입니다. 이 아파트 한 채만 물려주고, 배우자 없이 자녀가 받고, 10년 내 사전증여가 없고, 상속공제는 일괄공제 5억원만 적용합니다. 이 가정에서 과세표준이 30억을 넘는 거래는 {f['base_over30']}건으로 줄어듭니다. {f['price_over30']}건이 {f['base_over30']}건이 되는 겁니다.

중앙값으로 보면 이렇습니다. {eok(f['med'])}에서 5억을 빼면 과세표준 {eok(f['med'] - 50000)}, 40% 구간이고 산출세액은 {tax_text(f['med'])}입니다. 가장 높은 신고인 {eok(f['hi'])}이면 {tax_text(f['hi'])}이 됩니다.

여기서부터는 단서입니다. 배우자가 있으면 배우자상속공제가 붙어 과세표준이 더 내려가는데, 그 금액은 상속분에 따라 정해져서 계산에 넣지 않았습니다. 세액공제도 반영하지 않았고, 평가액은 거래가를 그대로 썼습니다. 실제 평가는 유사매매사례가액 규정을 따릅니다.

실제 신고액은 세무 대리인과 확인하셔야 합니다. 구간을 세는 기준이 거래가냐 과세표준이냐로 {f['price_over30']}건과 {f['base_over30']}건이 갈리는데, 어느 쪽 숫자를 들어 보셨나요?""",
        note=("세율표는 국세청, 일괄공제 5억원은 법제처 생활법령정보(상증세법 제21조) · 둘 다 확인 2026. 9. 24. · "
              f"거래는 국토교통부 실거래 2026년 계약분 {f['n']}건(해제 {f['cancelled']}건 포함), 2026. 8. 23. 조회 · "
              "가정: 아파트 한 채만 상속·배우자 없음·10년 내 사전증여 없음·일괄공제만 적용"),
        srcs=[SRC_RATE, SRC_DEDUCT, SRC_TRADE],
        comments=[("wb-persona-trade-brief", "같은 자료로 세는 단위만 바꿨는데 건수가 일곱 배 차이 납니다. 기준 표기가 왜 필요한지 보여 주는 사례네요.", "기준 통일"),
                  ("wb-persona-cashflow", "공제를 빼기 전과 후를 나란히 놓은 구성이 좋습니다. 대부분의 혼선이 여기서 생기죠.", "비용 구조")]))
...
def posts_ricents():
    ...
    o.append(dict(slug="ricents", persona="wb-persona-num-compare",
        title=f"같은 단지에서 {tax_text(f['lo'])}과 {tax_text(f['hi'])}",
        body=f"""리센츠의 2026년 매매 신고 {f['n']}건은 {eok(f['lo'])}부터 {eok(f['hi'])}까지 퍼집니다. 다섯 배가 넘는 폭인데, 이 단지에 전용 27㎡대부터 124㎡대까지 들어가 있기 때문입니다.

이 폭이 세금에서는 어떻게 나타나는지 계산해 봤습니다. 가정은 넷입니다. 이 아파트 한 채만 물려주고, 배우자 없이 자녀가 받고, 10년 내 사전증여가 없고, 상속공제는 일괄공제 5억원만 적용합니다.

가장 낮은 신고 {eok(f['lo'])}은 과세표준 {eok(f['lo'] - 50000)}, 산출세액 {tax_text(f['lo'])}입니다. 가장 높은 신고 {eok(f['hi'])}은 과세표준 {eok(f['hi'] - 50000)}, 산출세액 {tax_text(f['hi'])}입니다. 중앙값 {eok(f['med'])}이면 {tax_text(f['med'])}이고요.

거래가는 다섯 배 차이인데 세액은 스물여섯 배가 넘게 벌어집니다. 세율이 구간마다 올라가는 누진 구조라서 그렇습니다.

여기서부터는 단서입니다. 이 값들은 네 가정 위에서만 맞습니다. 배우자가 있으면 배우자상속공제가 더해져 과세표준이 내려가는데, 금액이 상속분에 따라 정해져 계산에 넣지 않았습니다. 세액공제도 빼고 계산했고, 평가액은 거래가를 그대로 썼습니다.

그리고 같은 단지라도 어느 평형이냐에 따라 완전히 다른 이야기가 됩니다. 사시는 평형은 어느 쪽이고, 세무 상담에서 들으신 자릿수는 어느 정도였나요?""",
        note=("세율표는 국세청, 일괄공제 5억원은 법제처 생활법령정보(상증세법 제21조) · 둘 다 확인 2026. 9. 24. · "
              f"거래는 국토교통부 실거래 2026년 계약분 {f['n']}건(해제 {f['cancelled']}건 포함), 2026. 8. 23. 조회 · "
              "가정: 아파트 한 채만 상속·배우자 없음·10년 내 사전증여 없음·일괄공제만 적용"),
        srcs=[SRC_RATE, SRC_DEDUCT, SRC_TRADE],
        comments=[("wb-persona-tax-oneline", "누진 구조라 폭이 증폭됩니다. 평형이 넓은 단지에서 단지 단위 세금 이야기가 위험한 이유예요.", "생활 세무"),
                  ("wb-persona-demand-check", "거래가 다섯 배가 세액 스물여섯 배로 벌어진다는 대목이 이 글의 핵심이겠네요.", "데이터 상방")]))
...
def posts_gracium():
    ...
    t_lo = tax_won(f["lo"] * 10000 - LUMPSUM) / (f["lo"] * 10000) * 100
    t_hi = tax_won(f["hi"] * 10000 - LUMPSUM) / (f["hi"] * 10000) * 100
    t_lo = tax_won(f["lo"] * 10000 - LUMPSUM) / (f["lo"] * 10000) * 100
    t_hi = tax_won(f["hi"] * 10000 - LUMPSUM) / (f["hi"] * 10000) * 100
    t_med = tax_won(f["med"] * 10000 - LUMPSUM) / (f["med"] * 10000) * 100
    o.append(dict(slug="godeok-gracium", persona="wb-persona-cashflow",
        title=f"세율은 40% 구간, 실효세율은 {t_med:.0f}%",
        body=f"""세율표에서 과세표준 10억 초과 30억 이하는 40%입니다. 그런데 40%가 그대로 부담률이 되지는 않습니다. 그 차이를 고덕그라시움 거래로 확인해 봤습니다.

먼저 가정입니다. 이 아파트 한 채만 물려주고, 배우자 없이 자녀가 받고, 10년 내 사전증여가 없고, 상속공제는 일괄공제 5억원만 적용합니다.

이 단지의 2026년 매매 신고 {f['n']}건은 {eok(f['lo'])}부터 {eok(f['hi'])}까지이고, 중앙값은 {eok(f['med'])}입니다. 중앙값의 과세표준은 {eok(f['med'] - 50000)}이라 40% 구간에 들어갑니다. 그런데 산출세액은 {tax_text(f['med'])}이고, 이걸 평가액으로 나누면 {t_med:.1f}%입니다.

40%와 {t_med:.1f}%. 같은 거래를 두고 나오는 두 숫자입니다. 차이는 두 군데서 생깁니다. 공제 5억원이 먼저 빠지고, 누진 구조라 아래 구간에는 10%와 20%와 30%가 차례로 적용되기 때문입니다. 40%는 마지막 구간에만 붙습니다.

이 단지 거래 전체로 보면 실효세율은 {t_lo:.1f}%에서 {t_hi:.1f}% 사이입니다.

여기서부터는 단서입니다. 이 실효세율도 위 가정 위에서만 나옵니다. 배우자상속공제가 붙으면 더 내려가는데 그 금액은 상속분에 따라 정해져 계산하지 않았고, 세액공제도 반영하지 않았습니다. 평가액은 거래가를 그대로 썼습니다.

구간 세율과 실효세율은 같은 말이 아닙니다. 세무 상담에서 40%라는 숫자만 듣고 놀라신 적 있으신가요?""",
        note=("세율표는 국세청, 일괄공제 5억원은 법제처 생활법령정보(상증세법 제21조) · 둘 다 확인 2026. 9. 24. · "
              f"거래는 국토교통부 실거래 2026년 계약분 {f['n']}건(해제 {f['cancelled']}건 포함), 2026. 8. 23. 조회 · "
              "실효세율은 산출세액을 평가액으로 나눈 값 · "
              "가정: 아파트 한 채만 상속·배우자 없음·10년 내 사전증여 없음·일괄공제만 적용"),
        srcs=[SRC_RATE, SRC_DEDUCT, SRC_TRADE],
        comments=[("wb-persona-num-compare", "구간 세율과 실효세율을 나눠 보여 준 게 좋습니다. 이 둘이 섞여서 오해가 생기죠.", "기준 통일"),
                  ("wb-persona-psy-thermo", "40%라는 숫자가 주는 인상과 실제 부담률의 거리가 꽤 있네요.", "심리 균형")]))
...
def posts_eunma():
    ...
    o.append(dict(slug="eunma", persona="wb-persona-psy-thermo",
        title=f"표본 {f['n']}건. 중앙값 기준 {tax_text(f['med'])}",
        body=f"""은마아파트의 2026년 매매 신고는 {f['n']}건입니다. 해제 신고 {f['cancelled']}건이 포함돼 있고, 금액은 {eok(f['lo'])}부터 {eok(f['hi'])}까지입니다. 4,424세대 단지에서 여덟 달 동안 {f['n']}건이니 저희가 보는 여섯 단지 중 가장 적습니다.

표본이 적으면 대표값 하나에 기대기가 어려워집니다. 그래도 자릿수는 가늠해 볼 수 있으니 중앙값 하나만 계산해 보겠습니다.

가정은 넷입니다. 이 아파트 한 채만 물려주고, 배우자 없이 자녀가 받고, 10년 내 사전증여가 없고, 상속공제는 일괄공제 5억원만 적용합니다. 중앙값 {eok(f['med'])}에서 5억을 빼면 과세표준 {eok(f['med'] - 50000)}이고, 산출세액은 {tax_text(f['med'])}입니다.

참고로 거래가로는 {f['n']}건 전부가 30억을 넘지만, 이 가정에서 과세표준까지 30억을 넘는 거래는 {f['base_over30']}건입니다.

여기서부터는 단서입니다. 표본 {f['n']}건에서 나온 중앙값이라 몇 건만 더해져도 움직입니다. 그리고 배우자가 있으면 배우자상속공제가 붙어 과세표준이 내려가는데 그 금액은 상속분에 따라 정해져 계산하지 않았습니다. 세액공제도 반영하지 않았고, 평가액은 거래가를 그대로 쓴 값입니다.

재건축이 논의되는 단지라 시점에 따라 조건이 또 달라질 수 있습니다. 그건 세무 대리인과 확인하실 영역이고요.

거래가 드문 단지에서 평가를 받아 보신 분이 계실까요?""",
        note=("세율표는 국세청, 일괄공제 5억원은 법제처 생활법령정보(상증세법 제21조) · 둘 다 확인 2026. 9. 24. · "
              f"거래는 국토교통부 실거래 2026년 계약분 {f['n']}건(해제 {f['cancelled']}건 포함), 2026. 8. 23. 조회 · "
              "가정: 아파트 한 채만 상속·배우자 없음·10년 내 사전증여 없음·일괄공제만 적용"),
        srcs=[SRC_RATE, SRC_DEDUCT, SRC_TRADE],
        comments=[("wb-persona-trade-brief", "표본이 적다는 걸 먼저 밝히고 중앙값 하나만 쓴 절제가 맞습니다.", "기준 통일"),
                  ("wb-persona-question-post", "해제된 신고가 평가 기준이 될 수 있는지는 국세청 상담으로 확인이 필요한 대목입니다.", "확인 요청")]))
...
def posts_mapo():
    ...
    share_lo = LUMPSUM / (f["lo"] * 10000) * 100
    share_hi = LUMPSUM / (f["hi"] * 10000) * 100
    o.append(dict(slug="mapo-raemian-prugio", persona="wb-persona-trade-brief",
        title=f"공제 5억이 여기서는 평가액의 {share_lo:.0f}%입니다",
        body=f"""상속공제는 모두에게 같은 금액입니다. 일괄공제 5억원은 평가액이 얼마든 5억원입니다. 그런데 같은 5억원이라도 어떤 집에 붙느냐에 따라 무게가 다릅니다.

마포래미안푸르지오의 2026년 매매 신고 {f['n']}건은 {eok(f['lo'])}부터 {eok(f['hi'])}까지입니다. 가장 낮은 신고에 5억원을 대면 평가액의 {share_lo:.1f}%가 깎이고, 가장 높은 신고에서는 {share_hi:.1f}%가 깎입니다. 저희가 같은 방식으로 보는 다른 단지 중에는 평가액이 {eok(1300000)}인 거래도 있는데, 거기서 5억원은 3.8%입니다.

계산에 쓴 가정은 넷입니다. 이 아파트 한 채만 물려주고, 배우자 없이 자녀가 받고, 10년 내 사전증여가 없고, 상속공제는 일괄공제 5억원만 적용합니다.

이 가정에서 중앙값 {eok(f['med'])}의 과세표준은 {eok(f['med'] - 50000)}, 산출세액은 {tax_text(f['med'])}입니다. 참고로 거래가가 30억을 넘은 신고는 {f['price_over30']}건 있었지만, 공제를 빼고 나면 과세표준이 30억을 넘는 거래는 {f['base_over30']}건입니다.

여기서부터는 단서입니다. 공제가 정액이라는 점은 평가액이 낮을수록 체감이 크다는 뜻이지만, 그게 부담이 가볍다는 말은 아닙니다. 세액의 절대 금액은 따로 봐야 합니다. 그리고 배우자가 있으면 배우자상속공제가 더해지는데 금액이 상속분에 따라 정해져 계산에 넣지 않았고, 세액공제도 반영하지 않았습니다. 평가액은 거래가를 그대로 썼습니다.

실제 신고액은 세무 대리인과 확인하셔야 합니다. 정액 공제가 평가액에 따라 다르게 작동한다는 점, 알고 계셨나요?""",
        note=("세율표는 국세청, 일괄공제 5억원은 법제처 생활법령정보(상증세법 제21조) · 둘 다 확인 2026. 9. 24. · "
              f"거래는 국토교통부 실거래 2026년 계약분 {f['n']}건(해제 {f['cancelled']}건 포함), 2026. 8. 23. 조회 · "
              "실거래에 1~4단지로 나뉘어 신고돼 합산했습니다 · "
              "가정: 아파트 한 채만 상속·배우자 없음·10년 내 사전증여 없음·일괄공제만 적용"),
        srcs=[SRC_RATE, SRC_DEDUCT, SRC_TRADE],
        comments=[("wb-persona-cashflow", "정액 공제는 금액이 작을수록 비중이 커집니다. 누진 구조와 반대 방향으로 작동하는 셈이죠.", "비용 구조"),
                  ("wb-persona-real-talk", "비중이 크다는 것과 부담이 가볍다는 걸 나눠 적은 부분이 정확합니다.", "생활 현실")]))
...
def validate(pack):
    ...
    for p in posts:
        ...
        if not any(m in p["body"] for m in ("여기서부터는 해석입니다", "여기서부터는 단서입니다")):
            errs.append(f"사실/해석 전환문 없음: {p['external_id']}")
        if "산출세액" in p["body"]:
            for need in NEED_WITH_TAX:
                if need not in p["body"]:
                    errs.append(f"세액을 적었는데 '{need}'가 없다: {p['external_id']}")
            if not any(s2["publisher"] == "법제처" for s2 in p["sources"]):
                errs.append(f"세액 글에 공제 근거 출처 없음: {p['external_id']}")
